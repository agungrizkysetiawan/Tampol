...
class myPDF extends FPDF {
    function header()
    {
        ...
        $this->SetFont('Arial','B',14);
        $this->Cell(120);
        $this->Cell(30,5,'DATA PELANGGARAN ( SUDAH DITINDAKLANJUTI )',0,1,'C');
    ...
    function headerTable(){
        ...
        $this->Cell(148, 10, 'PELANGGAR', 1,0,'C');
        $this->SetY(-135.9);
        $this->Cell(6);
    ...
    function viewTable($db){
        ...
        $stmt   =   mysqli_query($db, "SELECT * FROM data_pelanggaran WHERE status = 'Y'");
        $no     =   1;
